...
if(isset($_POST['add_cart'])){
    if(isset($_SESSION['carrito'])){
        ...
        if(!in_array($_GET['id'],$session_array_id)){
            ...
            //se busca la id del articulo a actulizar la cantidad la cual depende del maximo de stock que hay
            
            $limite=$_POST['limite_cantidad'];
            $suma=$_POST['cantidad']+$session_array_cantidad[0];
            if($suma>$limite){
                for($i=0;$i<count($_SESSION['carrito']);$i++){
                    if($_SESSION['carrito'][$i]['id'] == $_GET['id']){
                        $_SESSION['carrito'][$i]['cantidad'] = $limite;
                    }
                }
                ...
                for($i=0;$i<count($_SESSION['carrito']);$i++){
                    if($_SESSION['carrito'][$i]['id'] == $_GET['id']){
                        $_SESSION['carrito'][$i]['cantidad'] = $suma;
                    }
                }   
            }
        }
...
<body>
    ...
    <div id="contenido" class="container">
        <h2 class="text-center">Lista del carrito</h2>
        <br>
        <!--Los datos seran mostrados en una tabla-->
                <table class="table table-bordered table-striped text-center">
                    <thead>
                        <tr>
                            <th scope="col">Imagen</th>
                            <th scope="col" class="d-none d-lg-table-cell">Producto</th>
                            <th scope="col" class="d-none d-lg-table-cell">Precio</th>
                            <th scope="col" class="d-none d-lg-table-cell">Cantidad</th>
                    ...
                    <tbody>
                        <?php
                                //se crea la variable total y se inicializa a 0
                                $total=0;
                                //se comprueba que el carrito no este vacío, si no lo esta mostrará los artículos en vez de solo los th de thead
                                if(!empty($_SESSION['carrito'])){
                                    //se muestran los datos de cada artículo que hay en carrito
                        ...
                        <tr>
                            ...
                            <td class="d-none d-lg-table-cell"><?php echo $value['nombre'];?></td>
                            <td class="d-none d-lg-table-cell"><?php echo $value['precio'];?>€</td>
                            <td class="d-none d-lg-table-cell"><?php echo $value['cantidad'];?></td>
                            <td ><?php echo number_format($value['precio']*$value['cantidad'],2);?>€</td>
                            <!--El enlace enviara a eliminarcarrito.php ademas de un get donde se comprobara si es remove o id el cual guardará el valor id del artículo-->
                        ...
                        <?php
                                        //se van sumando la cantidad*precio de cada producto mas el anterior resultado
                        ...
                        <tr>
                            ...
                            <td class="d-none d-lg-table-cell"><b>Total</b></td>
                            <td><?php echo number_format($total,2);?>€</td>
                            <td>
                                <!--Se comprueba que si no exite la sesion de un usuario logeado mostrará un boton con una función que se ejecutará al hacer click y mostrará un mensaje de que debes iniciar sesión antes de realizar la compra-->
                                <?php if(!isset($_SESSION['id'])){ ?>
                                <button class="btn btn-primary " onclick="alerta()">Comprar</button>
                                <?php
                                    }else{
                                    ?>
                                <!--Si exite la sesión se enviará al a hacerpedido.php-->
                                <a href="./hacerpedido.php?action=limpiarcarro">
